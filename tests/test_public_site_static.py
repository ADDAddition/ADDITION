...
class PublicSiteStaticTests(unittest.TestCase):
    ...
    def test_join_docs_state_live_bootstrap_and_sync(self) -> None:
        ...
        self.assertNotIn("--data-dir <dir>", join)
        self.assertIn(r"$HOME\addition-testnet", join)
        self.assertIn("addition-wallet-testnet.exe", join)
        self.assertIn(".\\addition-wallet-testnet.exe --cli getinfo", join)
    ...
    def test_download_page_is_testnet_local_only(self) -> None:
        ...
        self.assertIn('href="/download/addition-wallet-testnet.exe"', page)
        self.assertIn('href="/download/addition-wallet-cli-testnet.exe"', page)
        self.assertIn("addition-wallet-testnet.exe", page)
        self.assertIn("addition-wallet-cli-testnet.exe", page)
        self.assertIn(".\\addition-wallet-testnet.exe --cli getinfo", page)
        self.assertIn(r"--data-dir $HOME\addition-testnet", page)
        self.assertIn("127.0.0.1:8545", page)
        self.assertIn("chmod +x", page)
        self.assertIn("--data-dir $HOME/addition-testnet", page)
        self.assertIn("Windows (testnet / local)", page)
        self.assertIn("Linux (testnet / local)", page)
        self.assertIn("web/public/download/", page)
        self.assertIn("refuses non-loopback", page)
        self.assertIn("Cashiers download", page)
    ...
    def test_readme_linux_compile_path(self) -> None:
        ...
        self.assertIn("addition-wallet-testnet.exe", readme)
        self.assertIn(".\\addition-wallet-testnet.exe --cli getinfo", readme)
        self.assertIn(r"$HOME\addition-testnet", readme)
...
"""
    ...
    def test_raw_markdown_docs_are_fetchable(self) -> None:
        ...
        self.assertIn(r"$HOME\addition-testnet", join_md)
        self.assertIn("addition-wallet-testnet.exe", join_md)
        self.assertIn(".\\addition-wallet-testnet.exe --cli getinfo", join_md)
        ...
        self.assertIn("/download/*.exe", headers)
        self.assertIn("application/octet-stream", headers)
        ...
        self.assertEqual(
            serve.content_type(Path("addition-wallet-testnet.exe")),
            "application/octet-stream",
        )
        self.assertEqual(
            serve.content_type(Path("addition-wallet-cli-testnet.exe")),
            "application/octet-stream",
        )
        ...
        try:
            ...
            with urllib.request.urlopen(f"http://{host}:{port}/download/") as resp:
                self.assertIn("text/html", resp.headers.get("Content-Type", ""))
                download_body = resp.read().decode("utf-8")
                self.assertIn("addition-wallet-testnet", download_body)
                self.assertIn("/download/addition-wallet-testnet.exe", download_body)
                self.assertIn(".\\addition-wallet-testnet.exe --cli getinfo", download_body)
        finally:
            server.shutdown()
            server.server_close()
