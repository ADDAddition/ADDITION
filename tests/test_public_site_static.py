...
class PublicSiteStaticTests(unittest.TestCase):
    ...
    def test_chrome_is_mainnet_product_nav(self) -> None:
        ...
        self.assertIn('["/compare/", "Compare"]', chrome)
        self.assertIn('["/privacy/", "Privacy"]', chrome)
        self.assertIn('["/download/", "Download"]', chrome)
        self.assertIn('["/whitepaper/", "Whitepaper"]', chrome)
        ...
        self.assertIn("contact@additionblockchain.com", chrome)
        self.assertIn('href="/compare/"', chrome)
        self.assertNotIn('["/launch/", "Launch"]', chrome)
  ...
  .hero-stinger {
    ...
    def test_compare_page_is_live_only(self) -> None:
        ...
        benchmark = read("benchmark/index.html")
        self.assertIn("ADDITION_MAINNET_V1", compare)
        self.assertIn("memory_hard", compare)
        self.assertIn("ML-DSA-87", compare)
        self.assertIn("opening_not_zk", compare)
        self.assertIn("Bitcoin", compare)
        self.assertIn("Ethereum", compare)
        self.assertIn("Solana", compare)
        self.assertIn("Monero", compare)
        ...
        self.assertNotIn("ADDITION_FAST_V1", compare)
        self.assertNotIn("scaffold", compare.lower())
        self.assertNotIn("coming when proven", compare.lower())
        self.assertNotIn("SmartChain", compare)
        self.assertNotRegex(compare.lower(), r"\bhonest\b")
        # No invented ADDITION TPS race vs Solana
        self.assertNotIn("100000", compare)
        self.assertNotIn("beats Solana", compare.lower())
        self.assertIn('/compare/', benchmark)
