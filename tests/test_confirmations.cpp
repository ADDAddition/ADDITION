...
int main() {
    {
        ...
        if (mainnet.min_difficulty_target != 0x000000FFFFFFFFFFULL ||
            mainnet.initial_difficulty_target != 0x000000FFFFFFFFFFULL ||
            mainnet.max_difficulty_target != 0x000000FFFFFFFFFFULL) {
            std::cerr << "test failed: ADDITION_MAINNET_V1 difficulty must stay 0x000000FFFFFFFFFF\n";
            return 1;
        }
    }
...

    {
        ...
        if (!addition::apply_cli_args(2, argv, ncfg, help, err)) {
            std::cerr << "test failed: --regtest CLI: " << err << '\n';
            return 1;
        }
        if (ncfg.mode != addition::NetworkMode::Regtest ||
            ncfg.chain.network_id != addition::kRegtestNetworkId ||
            ncfg.chain.network_mode != "regtest" ||
            ncfg.chain.pow_profile != "regtest" ||
            ncfg.chain.initial_difficulty_target != addition::kRegtestMinDifficultyTarget ||
            ncfg.data_dir != "data-regtest") {
            std::cerr << "test failed: --regtest profile\n";
            return 1;
        }
        for (const auto& peer : ncfg.bootstrap_peers) {
            if (peer == addition::kOperatorPublicP2p) {
                std::cerr << "test failed: --regtest bootstraps the public testnet seed\n";
                return 1;
            }
        }
    }

    {
        ...
        if (addition::apply_cli_args(3, argv, ncfg, help, err)) {
            std::cerr << "test failed: --regtest --mainnet must be rejected\n";
            return 1;
        }
    }
