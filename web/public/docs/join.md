...
Pages under `/local/` (`/wallet/`, `/tokens/`, `/swap/`, `/privacy/`, `/evm/`) talk to `/local-rpc` → `127.0.0.1:8545`. On the public host they stay **RPC offline**.

Local forms: `/local/` hub, `/tokens/` (`token_create`), `/swap/` (`fee_bps=0` allowed), `/wallet/` (mine / stake), `/privacy/` (`claim=opening_not_zk`), `/evm/` (send disabled). Floor fee is `min_fee=0` on current `main`. Spend signatures are ML-DSA-87. Public read cannot write.
