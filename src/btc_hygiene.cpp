...
BtcScriptClass script_class_from_name(const std::string& value) {
    if (value == "p2pk") {
...
bool parse_hygiene_receipt_body(const std::string& body, BtcHygieneReport& out, std::string& error) {
    ...
    while (std::getline(iss, part, '|')) {
        ...
        if (key == "btc_addr") {
            ...
            out.script_class = script_class_from_name(value);
        } else if (key == "reuse") {
            ...
            saw_claim = true;
            if (value != "attestation_not_bip360") {
...
bool split_hygiene_receipt(const std::string& note,
                           std::string& body,
                           std::string& pubkey,
    ...
    if (sig_pos == std::string::npos || pub_pos == std::string::npos || pub_pos > sig_pos) {
        error = "receipt missing attestation fields";
        return false;
    }
    body = note.substr(0, pub_pos);
    pubkey = note.substr(pub_pos + 12, sig_pos - (pub_pos + 12));
    signature = note.substr(sig_pos + 12);
    if (body.empty() || pubkey.empty() || signature.empty()) {
        error = "receipt missing attestation fields";
        return false;
    }
    return true;
...
std::string assemble_hygiene_receipt(const std::string& body,
                                     const std::string& pubkey,
                                     const std::string& signature) {
    return body + "|attest_pub=" + pubkey + "|attest_sig=" + signature;
}
